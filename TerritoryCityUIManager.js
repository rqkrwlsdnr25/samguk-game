(function(global){
  ...
  function drawMapCitiesWithIslandCheck(ctx,cityList,currentZoom){
    ...
    for(const city of cityList){
      const territoryId=Number(city?.territoryId??city?.id);
      ...
      if(showTroopLabel&&city?.count!==undefined&&city?.count!==null){
  ...
  global.SAMGUK_TERRITORY_CITY_UI=controller;
  global.SAMGUK_CITY_SPRITES=controller; // backward-compatible alias
  global.drawMapCities=drawMapCities; // Canvas-compatible reference renderer
  global.drawMapCitiesWithIslandCheck=drawMapCitiesWithIslandCheck;
})(window);
